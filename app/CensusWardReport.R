...
rank <- civisdata[, c(1:10, 271:286)]
rank <- as.data.table(rank)

#Create the weighted response value
#NOTE: what's the best way to come up with this? These few lines need more work
rank$handicap <- (rank$low_response_score/(mean(rank$low_response_score, na.rm = TRUE)))
rank$weightedresponse <- (rank$mail_return_rate_cen_2010*rank$handicap)
...
rankdt<- rank[, .(mean_response = mean(mail_return_rate_cen_2010, na.rm = TRUE), 
